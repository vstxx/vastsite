...
test('routes to local releases and the complete legal policy set', async () => {
  ...
  assert.equal(releaseManifest.releases[0].version, '0.3.0');
  assert.equal(releaseManifest.releases[0].channel, 'Official');
  assert.equal(releaseManifest.releases[0].previousVersion, '0.2.7');
  assert.match(releaseManifest.releases[0].files[0].url, /vast-public\/releases\/download\/v0\.3\.0/);
  ...
  assert.match(changelog, /PDF Open with/);
  assert.match(changelog, /Corner radius/);
  assert.match(changelog, /native ad-blocking engine/);
  assert.match(changelog, /next-launch/);
  assert.match(changelog, /Electron 44\.1\.0/);
  assert.match(changelog, /Microsoft Store/);
  assert.match(changelog, /0\.2\.7-to-0\.3\.0/);
  assert.equal(releaseManifest.releases[1].version, '0.2.7');
  assert.equal(releaseManifest.releases[1].channel, 'Legacy');
  assert.match(releaseManifest.releases[1].files[0].url, /vast-public\/releases\/download\/v0\.2\.7/);
  assert.equal(releaseManifest.releases[2].version, 'Beta 0.2.5');
  assert.equal(releaseManifest.releases[2].channel, 'Legacy');
  assert.match(releaseManifest.releases[2].files[0].url, /vast-public\/releases\/download\/v0\.2\.5/);
  assert.equal(releaseManifest.releases[3].version, 'Beta 0.1.5');
  assert.match(releaseManifest.releases[3].files[0].url, /vast-public\/releases\/download\/public-release-0\.1\.5/);
  ...
  assert.match(releases, /Download Updater/);
  assert.match(releases, /not Store or Portable/);
  assert.match(releases, /available version may differ/);
